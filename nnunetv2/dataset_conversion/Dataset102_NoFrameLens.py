...
if __name__ == "__main__":
    ...
    source = "/root/autodl-tmp/dataset/noframe"
    nnUNet_raw = "/root/autodl-tmp/data/_raw"
    dataset_name = "Dataset102_NoFrameLens"
    ...
    with multiprocessing.get_context("spawn").Pool(8) as p:
        ...
        valid_ids = subfiles(
            join(train_source, "mask", "lens"), join=False, suffix="png"
        )
        ...
        # test set
        valid_ids = subfiles(
            join(test_source, "mask", "lens"), join=False, suffix="png"
        )
        for v in valid_ids:
            r.append(
                p.starmap_async(
